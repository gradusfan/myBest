...
namespace FanSwProject
{
    public partial class Form_crameProcess : Form
    {
        ...
        {
            string strRes = "Form_crameProcess";
        ...
        private void bnOpen_Click(object sender, EventArgs e)
        {
            ...
            LogRecord.WriteOperLog("相机打开成功");
            nRet = m_MyCamera.OpenDevice();
            if (CErrorDefine.MV_OK != nRet)
            {
                m_MyCamera.DestroyHandle();
                ShowErrorMsg("Device open fail!", nRet);
                LogRecord.WriteOperLog("相机打开失败");
                return;
            }
            ...
            if (device.nTLayerType == CSystem.MV_GIGE_DEVICE)
            {
                ...
                if (nPacketSize > 0)
                {
                    ...
                    if (nRet != CErrorDefine.MV_OK)
                    {
                        ...
                        LogRecord.WriteOperLog("Set Packet Size failed");
                    }
                }
            }
        ...
        private void bnStopGrab_Click_1(object sender, EventArgs e)
        {
            // ch:标志位设为false | en:Set flag bit false
            m_bGrabbing = false;
            m_hReceiveThread.Join();
        ...
        private void cbSoftTrigger_CheckedChanged_1(object sender, EventArgs e)
        {
            if (cbSoftTrigger.Checked)
            {
                // ch:触发源设为软触发 | en:Set trigger source as Software
        ...
        private void bnTriggerExec_Click_1(object sender, EventArgs e)
        {
            // ch:触发命令 | en:Trigger command
            int nRet = m_MyCamera.SetCommandValue("TriggerSoftware");
            if (CErrorDefine.MV_OK != nRet)
        ...
        private void bnStartRecord_Click_1(object sender, EventArgs e)
        {
            if (false == m_bGrabbing)
            {
                ShowErrorMsg("Not Start Grabbing", 0);
        ...
        private void bnStopRecord_Click_1(object sender, EventArgs e)
        {
            if (false == m_bGrabbing)
            {
                ShowErrorMsg("Not Start Grabbing", 0);
        ...
        private void bnSaveBmp_Click_1(object sender, EventArgs e)
        {
            if (false == m_bGrabbing)
            {
                ShowErrorMsg("Not Start Grabbing", 0);
        ...
        private void bnSaveJpg_Click_1(object sender, EventArgs e)
        {
            if (false == m_bGrabbing)
            {
                ShowErrorMsg("Not Start Grabbing", 0);
        ...
        private void bnSaveTiff_Click_1(object sender, EventArgs e)
        {
            if (false == m_bGrabbing)
            {
                ShowErrorMsg("Not Start Grabbing", 0);
        ...
        private void bnSavePng_Click_1(object sender, EventArgs e)
        {
            if (false == m_bGrabbing)
            {
                ShowErrorMsg("Not Start Grabbing", 0);
            ...
            lock (BufForDriverLock)
            {
                ...
                stSaveFileParam.Quality = 9;
                stSaveFileParam.MethodValue = 2;
                stSaveFileParam.ImagePath = "D:/siren资料/FanSwProject/FanSwProject/crama_photo/"+"Image_w" + stSaveFileParam.Image.Width.ToString() + "_h" + stSaveFileParam.Image.Height.ToString() + "_fn" + m_pcImgSpecInfo.FrameNum.ToString() + ".png";
        ...
        {
            //if (!Directory.Exists(@"D:\siren资料\FanSwProject\FanSwProject\crama_photo"))
            //{
            //    //创建路径
            //    Directory.CreateDirectory(@"D:\siren资料\FanSwProject\FanSwProject\crama_photo");
            //}
